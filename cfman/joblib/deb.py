...
def installed(ctx, pkg):
    res = ctx.run(deb.Dpkg(pkg).status().pipe(file.Grep('Status:').case()), warn=True)
...
def apt_key_add(ctx, fingerprint, filename=None, url=None, keyserver=None):
    g = gpg.Gpg(
        '--ignore-time-conflict',
        '--no-options',
        '--no-default-keyring',
    ...
    if fingerprint is None:
        if filename is not None:
            ctx.run(deb.AptKey().add(filename), warn=True, hide=True)
        elif url is not None:
            with file_job.temp_dir(ctx, 'fabtools-gpghome.XXXXXXXXXX') as tmp_dir:
                ctx.run(ctx, file.Wget(url).output(os.path.join(tmp_dir, 'key.gpg')), warn=True, hide=True)
                ctx.run(ctx, g.keyring(os.path.join(tmp_dir, 'key.gpg')).homedir(tmp_dir).export(asc=True).pipe(deb.AptKey().add('-')), warn=True, hide=True)
        else:
            raise ValueError(
        ...
        if filename is not None:
            ...
            with file_job.temp_dir(ctx, 'fabtools-gpghome.XXXXXXXXXX') as tmp_dir:
                if file_job.exists(ctx, '/etc/apt/trusted.db'):
                    ctx.run(file.Chmod('/etc/apt/trusted.db', 'o=r'))
                ctx.run(file.Wget(url).output(os.path.join(tmp_dir, 'key.gpg')), warn=True, hide=True)
                ctx.run(g.keyring(os.path.join(tmp_dir, 'key.gpg')).homedir(tmp_dir).export(asc=True).pipe(deb.AptKey().add('-')), warn=True, hide=True)
        else:
